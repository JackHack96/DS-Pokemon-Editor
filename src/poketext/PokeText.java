...
public class PokeText {
	...
	public static void readText(FimgEntry fs, ArrayList<String> textList) throws Exception
	{
		ByteArrayInputStream inp=new ByteArrayInputStream(fs.getEntryData());
		int stringCount;
		boolean flag=false, flag2=false;
		try {
			stringCount=EndianUtils.readSwappedUnsignedShort(inp);
			originalKey=EndianUtils.readSwappedUnsignedShort(inp);
			...
			for(int i=0;i<stringCount;i++)
			{
				num2=(num*(i+1))&0xffff;
				num3=num2 | (num2<<16);
				numArray[i]=(int) EndianUtils.readSwappedUnsignedInteger(inp);
				numArray[i]=numArray[i] ^ num3;
				numArray2[i]=(int) EndianUtils.readSwappedUnsignedInteger(inp);
			...
			for(int j=0;j<stringCount;j++)
			{
				...
				{
					num4=EndianUtils.readSwappedUnsignedShort(inp);
					num4=(num4^num);
				...
				textList.add(new String(text.getBytes(),"UTF-8"));
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
